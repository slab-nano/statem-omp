#!/usr/bin/env bash
# Hidden grader for the sats project. Usage: score_sats.sh <dir>
set -uo pipefail
...
echo "== functional (all 20, exact names) =="
if $PY - "$S" >/tmp/ss.log 2>&1 <<'PYEOF'
import sys, math
...
ok &= abs(mean_anomaly(1,2,3)-7)<1e-9
ok &= abs(orbital_period(7000)- (2*math.pi*math.sqrt(7000**3/398600.4418)))<1e-3
...
PYEOF
then echo "PASS  all 20 present with correct behavior"; pass=$((pass+1)); else echo "FAIL  $(tail -1 /tmp/ss.log)"; fail=$((fail+1)); fi
